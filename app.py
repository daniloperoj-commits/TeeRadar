    ...
    if lat_ref is not None and lon_ref is not None and radio_km is not None:
        ...
        if lat_ref_float is not None and lon_ref_float is not None and radio_km_float is not None:
            for campo in campos:
                ...
                campos_con_coordenadas_validas.append(campo)

                if not campo_dentro_bounding_box(
                    lat_ref_float,
                    lon_ref_float,
                    lat_campo,
                    lon_campo,
            ...
            if modo_debug:
                st.write("🧪 DEBUG DISTANCIAS", {
                    "campos_iniciales": len(campos),
                    "campos_con_coordenadas_validas": len(campos_con_coordenadas_validas),
                    "campos_tras_bounding_box": len(campos_en_bounding_box),
                    "campos_tras_haversine": len(campos_en_radio),
...
if st.button("Buscar"):
    ...
    if "zonas" in localidad_obj:
        if zona_seleccionada is None:
            st.error("Debes seleccionar una zona o distrito.")
            st.stop()
